...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT * FROM attendancerecord WHERE FirstName LIKE '%$search%' ";

    $result = mysqli_query($conn, $query);
    if (mysqli_num_rows($result) > 0) {
        $participants = mysqli_fetch_all($result, MYSQLI_ASSOC);
    } else {
        $message = "No attendancerecord found.";
    }
}
?>
...
<html>
<head>
    <title>Search attendancerecord</title>
	<meta charset="UTF-8">
...
<h2>
    ...
    <?php if (isset($participants)) { ?>
        ...
        <?php foreach ($participants as $participant) { ?>
            <p><?php echo $participant["FirstName"] . " " . $participant["Date"] . " " . $participant["TimeAttendance"]  . " " . $participant["TimeToLeave"] . " " . $participant["Absence"]; ?></p>
